...
import { VALIDATOR_REQUIRE, VALIDATOR_MIN, VALIDATOR_MAX } from '../../shared/components/util/validators';
...
const Withdraw = () => {
    ...
    const balanceUpdateHandler = event => {
        ...
        formState.inputs.balance.value = Number(identifiedTransaction.balance) - Number(formState.inputs.balance.value) ;
 ...
 return <form className="place-form" onSubmit={balanceUpdateHandler}>
     <Input 
        id='title' 
        element="input" 
        type="text" 
     ...
     <Input 
        ...
        validators={[VALIDATOR_MAX(identifiedTransaction.balance), VALIDATOR_MIN(1)]}
        errorText="Plase withdraw at least 1USD or a value lower or equal to yout balance amount"
        onInput={inputHandler}
        initialValue={formState.inputs.balance.value}
        initialValid={formState.inputs.balance.isValid}
     />

    <Button type="submit" disabled={!formState.isValid}>WITHDRAW</Button>
